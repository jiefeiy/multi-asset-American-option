function V0 = pricing_qmc_preint(p, type, ilevel, M_preint, M_qmc, beta, L)
% pricing using preintegration strategy and QMC with scramble Sobol sequence
disp('--------------------------------------------------')
disp('pricing using preintegration strategy and QMC with scramble Sobol sequence');
dt = p.expiration./p.numTimeStep;
disc = exp(-p.rate*dt);
MaxDepth = ilevel;                                         % interpolation level
num_sobol = M_qmc;
preintLevel = M_preint-1;
% bubble function 
bubble = @(x) prod(1-x.^2).^beta;
% compute transformation                                                           % scale parameter
[Q, Lambda] = eig(p.volatility^2*p.correlation);
[d, ind] = sort(diag(Lambda),'descend');
Q = Q(:, ind); Lambda = diag(d);                  % eigen pairs of \Sigma*P*\Sigma^\top
% The order of eigenvalues will impact the performance of QMC. 
...
%% quadrature knots %%% 
% generate Sobol seq. of dim (d-1)
sobol = sobolset(p.dim-1, 'Skip', 1e3, 'Leap', 1e2); 
sobol = scramble(sobol, 'MatousekAffineOwen');
pts = sobol(1:num_sobol,:)'; clear sobol;
presobol = qmean(2:end) + qstd(2:end,2:end)*norminv(pts);
% generate 1d quad pts
yknots = @(n) knots_normal(n,qmean(1),qstd(1,1));
yS = smolyak_grid(1, preintLevel,yknots,@lev2knots_lin); % dim 1 quadratue knots with normal density
preknots = yS.knots;
num_hermite = yS.size;
num_qknots = num_sobol*num_hermite;
preweights = yS.weights;
% pts next step
qknots = zeros(p.dim, num_qknots);
qknots(1,:) = reshape(repmat(preknots,num_sobol,1),[1,num_qknots]);
for n = 1:num_hermite
    qknots(2:end,((n-1)*num_sobol+1) : (n*num_sobol)) = presobol;
end
qweights = reshape(repmat(preweights,num_sobol,1), [1, num_qknots])/num_sobol;
...
for k = p.numTimeStep-1:-1:0 
    udata_in = disc*(qweights*payoff).*zbubble; 
