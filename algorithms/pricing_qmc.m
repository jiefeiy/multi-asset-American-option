function V0 = pricing_qmc(p, type, ilevel, M, beta, L)
% pricing using QMC with scramble Sobol sequence
disp('--------------------------------------------------')
disp('pricing using QMC with scramble Sobol sequence');
...
%% quadrature knots %%% Sobol seq.
sobol = sobolset(p.dim, 'Skip', 1e3, 'Leap', 1e2);
sobol = scramble(sobol, 'MatousekAffineOwen');
pts = sobol(1:num_qknots,:)'; clear sobol;
% pts next step
xqknots = zeros(p.dim, num_qknots, num_iknots_in);
for n = 1:num_iknots_in
    xqknots(:,:,n) = xknots_in(:,n) + qmean + qstd*norminv(pts);
end
...
for k = p.numTimeStep-1:-1:0 
    udata_in = disc*(mean(payoff)).*zbubble; 
