function Sr=myreduce_sparse_grid(S,tol)
...
[kk_in_ordered,i_in] = mysortrows(kk_in, Tol);
...
dkk_ordered_in = diff(kk_in_ordered,1,1);
...
invindex_in = zeros(length(i_in),1);
...
Sr.knots=kk_reduced';
Sr.knots_in = kk_in_reduced';
Sr.m=index_reduced;
% Sr.m_in = index_reduced_in;

nb_knots_reduced=length(index_reduced);
nb_knots_reduced_in = length(index_reduced_in);
Sr.n_in=invindex_in;
...
Sr.size_in = nb_knots_reduced_in;
