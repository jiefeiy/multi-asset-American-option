function S = mytensor_grid(N,m,knots)
...
pattern_in = generate_pattern_in(m,sz_in);

for n=1:N % loop over each dim
    [xx,ww] = knots{n}(m(n));
    S.knots_per_dim{n}=xx;
    S.knots(n,:) = xx(pattern(n,:));
    S.knots_in(n,:) = xx(pattern_in(n,:));
